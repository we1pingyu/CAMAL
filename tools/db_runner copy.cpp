...
#include "infrastructure/keylog.h"
...
environment parse_args(int argc, char * argv[])
{
    ...
    auto minor_opt = "minor options:" % (
        ...
        (option("--skew") & number("num", env.skew))
            % ("skewness for zipfian [0, 1)"),
        (option("--key-file").set(env.use_key_file, true) & value("file", env.key_file))
            % "use keyfile to speed up bulk loading",
        (option("--cache").set(env.use_cache, true) & number("cap", env.cache_cap))
            % "use block cache",
        (option("--key-log-file").set(env.use_key_log, true) & value("file", env.key_log_file))
            % "use keylog to record each key"
    );
    ...
    rocksdb::DB *& db)
{
    ...
    rocksdb_opt.num_levels = env.rocksdb_max_levels;
    // rocksdb_opt.bottommost_compression = rocksdb::kNoCompression;
    ...
    // fluid_compactor->OnFlushCompleted
    ...
    // }
    table_options.filter_policy.reset(rocksdb::NewBloomFilterPolicy(fluid_opt->bits_per_element));
    if (env.use_cache)
...
int run_random_non_empty_reads(environment env, rocksdb::DB * db, tmpdb::FluidOptions * fluid_opt, KeyLog * key_log)
{
    ...
    if (env.use_key_file)
    {
        // spdlog::info("KeyFileGenerator!!!!!");
        // spdlog::info(fluid_opt->num_entries);
    ...
    else
    {
        ...
        data_gen = new YCSBGenerator(fluid_opt->num_entries, env.dist_mode, env.skew);
    }
    ...
    {
        // auto key = data_gen->gen_existing_key();
...
int run_random_empty_reads(environment env, rocksdb::DB * db, tmpdb::FluidOptions * fluid_opt, KeyLog * key_log)
{
                    ...
                    rocksdb::DB * db, KeyLog * key_log)
{
    ...
    int hop = 10;
    spdlog::debug("Keys per range query : {}", hop);

    std::string value;
    std::mt19937 engine;
    // opencog::zipf_distribution<int, double> dist(existing_keys.size() - 1 - hop);
    ...
    for (size_t range_count = 0; range_count < env.range_reads; range_count++)
    {
        ...
        key_log->log_key(key);
        std::string limit = std::to_string(stoi(key) + hop);
        rocksdb::Iterator *it = db->NewIterator(rocksdb::ReadOptions());
        std::string value;
        for (it->Seek(key); it->Valid() && it->key().ToString() < limit;
             it->Next()) {
                       ...
                       rocksdb::DB * db, KeyLog * key_log)
{
    ...
    {
        std::pair<std::string, std::string> entry = data_gen->gen_new_kv_pair(fluid_opt->entry_size);
        // spdlog::info(entry.first);
        new_keys.push_back(entry.first);
        key_log->log_key(entry.first);
...
int prime_database(environment env,tmpdb::FluidOptions * fluid_opt, rocksdb::DB * db)
{
    rocksdb::ReadOptions read_opt;
    rocksdb::Status status;
    ...
    spdlog::info("Priming database with {} reads and writes", env.prime_reads);
    // for (size_t read_count = 0; read_count < env.prime_reads; read_count++)
    // {
    //     status = db->Get(read_opt, std::to_string(dist(engine)), &value);
    // }
    DataGenerator *data_gen = new YCSBGenerator(fluid_opt->num_entries, env.dist_mode, env.skew);
    for (size_t read_count = 0; read_count < env.non_empty_reads; read_count++)
    {
        // auto key = data_gen->gen_existing_key();
        // spdlog::info(key);
        ...
        // data_gen->gen_kv_pair(fluid_opt->entry_size);
        // db->Put(rocksdb::WriteOptions(), key_value.first, key_value.second);
    }
    // rocksdb::FlushOptions flush_opt;
    // flush_opt.wait = true;
    // flush_opt.allow_write_stall = true;
...
int main(int argc, char * argv[])
{
    ...
    if (env.prime_db)
    {
        prime_database(env, fluid_opt, db);
    }
    ...
    {
        empty_read_duration = run_random_empty_reads(env, db, fluid_opt,key_log); 
    }
    rocksdb_opt.statistics->getTickerMap(&stats);
    uint64_t z0_hit = stats["rocksdb.block.cache.hit"];
    uint64_t z0_miss = stats["rocksdb.block.cache.miss"];
    double z0_hit_rate = (z0_hit == 0) ? 0 : double(z0_hit) / double(z0_hit + z0_miss);
    if (env.non_empty_reads > 0)
    {
        read_duration = run_random_non_empty_reads(env, db, fluid_opt,key_log);
    }
    rocksdb_opt.statistics->getTickerMap(&stats);
    uint64_t z1_hit = stats["rocksdb.block.cache.hit"] - z0_hit;
    uint64_t z1_miss = stats["rocksdb.block.cache.miss"] - z0_miss;
    double z1_hit_rate = (z1_hit == 0) ? 0 : double(z1_hit) / double(z1_hit + z1_miss);
    if (env.range_reads > 0)
    {
        range_duration = run_range_reads(env, fluid_opt, db, key_log);
    }
    rocksdb_opt.statistics->getTickerMap(&stats);
    uint64_t q_hit = stats["rocksdb.block.cache.hit"] - z0_hit - z1_hit;
    uint64_t q_miss = stats["rocksdb.block.cache.miss"] - z0_miss - z1_miss;
    double q_hit_rate = (q_hit == 0) ? 0 : double(q_hit) / double(q_hit + q_miss);
    if (env.writes > 0)
    {
        std::pair<int, int> inserts_duration = run_random_inserts(env, fluid_opt, fluid_compactor, db,key_log);
        write_duration = inserts_duration.first;
        compact_duration = inserts_duration.second;
    }
    rocksdb_opt.statistics->getTickerMap(&stats);
    uint64_t w_hit = stats["rocksdb.block.cache.hit"] - z0_hit - z1_hit - q_hit;
    uint64_t w_miss = stats["rocksdb.block.cache.miss"] - z0_miss - z1_miss - q_miss;
    double w_hit_rate = (w_hit == 0) ? 0 : double(w_hit) / double(w_hit + w_miss);
    ...
    spdlog::info("(z0_cache_hit_rate) : ({})", z0_hit_rate);
    spdlog::info("(z0_cache_hit) : ({})", z0_hit);
    spdlog::info("(z0_cache_miss) : ({})", z0_miss);

    spdlog::info("(z1_cache_hit_rate) : ({})", z1_hit_rate);
    spdlog::info("(z1_cache_hit) : ({})", z1_hit);
    spdlog::info("(z1_cache_miss) : ({})", z1_miss);

    spdlog::info("(q_cache_hit_rate) : ({})", q_hit_rate);
    spdlog::info("(q_cache_hit) : ({})", q_hit);
    spdlog::info("(q_cache_miss) : ({})", q_miss);

    spdlog::info("(w_cache_hit_rate) : ({})", w_hit_rate);
    spdlog::info("(w_cache_hit) : ({})", w_hit);
    spdlog::info("(w_cache_miss) : ({})", w_miss);
