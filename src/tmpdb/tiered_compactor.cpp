...
TieredBaseCompactor::TieredBaseCompactor(const TieredOptions tiered_opt, const rocksdb::Options rocksdb_opt)
    : tiered_opt(tiered_opt), rocksdb_opt(rocksdb_opt), rocksdb_compact_opt()
{
...
int TieredCompactor::largest_occupied_level(rocksdb::DB *db) const
{
...
CompactionTask *TieredCompactor::PickCompaction(rocksdb::DB *db, const std::string &cf_name, const size_t level_idx)
{

    this->meta_data_mutex.lock();
    size_t T = this->tiered_opt.size_ratio;
    ...
    {
        for (size_t i = 0; i < this->tiered_opt.levels; i++)
        {
            if ((i * (T - 1) + 1) <= level_idx && level_idx <= ((i + 1) * (T - 1)))
            {
                ...
                if (lvl_size > 2 * pow(T, i) * this->tiered_opt.buffer_size)
                {
...
void TieredCompactor::OnFlushCompleted(rocksdb::DB *db, const ROCKSDB_NAMESPACE::FlushJobInfo &info)
{
    int largest_level_idx = this->largest_occupied_level(db);
...
bool TieredCompactor::requires_compaction(rocksdb::DB *db)
{
    // this->meta_data_mutex.lock();
    int largest_level_idx = this->largest_occupied_level(db);
    // this->meta_data_mutex.unlock();
...
void TieredCompactor::CompactFiles(void *arg)
{
    ...
    ((TieredCompactor *)task->compactor)
        ->compactions_left_count--;
    return;
}

void TieredCompactor::ScheduleCompaction(CompactionTask *task)
{
    if (!task->is_a_retry)
    {
        this->compactions_left_count++;
    }
    this->rocksdb_opt.env->Schedule(&TieredCompactor::CompactFiles, task);
    return;
}

size_t TieredCompactor::estimate_levels(size_t N, double T, size_t E, size_t B)
{
    if ((N * E) < B)
    {
...
size_t TieredCompactor::calculate_full_tree(double T, size_t E, size_t B, size_t L)
{
