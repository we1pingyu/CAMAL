...
class LevelCost(object):
    ...
    def run(self):
        ...
        for workload in workloads:
            ...
            if temp > 2:
                T_list.append(random.randint(2, temp - 1))
            if temp < estimate_T(N, M / 2 / 8, 1):
                T_list.append(random.randint(temp + 1, estimate_T(N, M / 2 / 8, 1)))
            print(T_list)
            # continue
            z0, z1, q, w = workload
            ratio = 1.0
            ...
            #     k += 1
            if temp > 1:
                h_list.append(random.randint(1, temp - 1))
            if temp < 16:
                h_list.append(random.randint(temp + 1, 16))
            print(h_list)
            for h in h_list:
                buffer = ratio * M - h * N
                size_ratio = T0
        ...
        pd.DataFrame(df).to_csv('raw_data/camal_xgb_level_lim.csv')
