        ...
        for workload in workloads:
            for size_ratio in size_ratios:
                for ratio in [0.25, 0.5, 0.75, 1.0]:
                    ...
                    # print(row)
                    df.append(row)
                    pd.DataFrame(df).to_csv('raw_data/tier_cost_lr_ckpt.csv')
                    iter += 1

        self.logger.info('Exporting data from tier cost lr')
        pd.DataFrame(df).to_csv('raw_data/tier_cost_lr.csv')
        self.logger.info(f'Finished tier_cost_lr, use {time.time()-start_time}s\n')
