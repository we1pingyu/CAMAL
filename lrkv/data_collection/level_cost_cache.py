...
class LevelCostDist(object):
    ...
    def run(self):
        ...
        for i in range(512):
            ...
            # n = 1e7
            size_ratio = random.choice(range(4, 10))
            # size_ratio = 5
            z0 = random.random()
            z1 = random.random()
            q = random.random()
            ...
            # workload = [0.0, 0.0, 0.0, random.random()]
            ...
            # cache_cap = 7
            cache_cap = cache_cap * 8 * 1024 * 1024  # *16MB
            z0 = int(np.ceil(num_queries * workload[0]))
            z1 = int(np.ceil(num_queries * workload[1]))
            q = int(np.ceil(num_queries * workload[2]))
            w = int(np.ceil(num_queries * workload[3]))
            ...
            self.de.export_csv_file(
                pd.DataFrame(df),
                'level_cost_cache_test_ckpt.csv',
            )

        self.logger.info('Exporting data from level cost')
        df = pd.DataFrame(df)
        self.de.export_csv_file(df, 'level_cost_cache_test.csv')
        self.logger.info('Finished level_cost_cache_test\n')
