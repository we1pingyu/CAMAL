    ...
    def run(self):
        ...
        for workload in workloads:
            ...
            if len(df) == 0:
                ...
                Wc, W = iter_model(df, "level", E, M, N)
                t = traverse_for_T(
                    [W], [Wc], z0, z1, q, w, E, M, N, n=-1, policy="level"
                )
                T_list = [temp]
            ...
            min_err = 1e9
            for h in range(2, 11):
                err = h_mbuf_level_equation(h * N, z0, z1, q, w, T0, E, M, N)
                if err < min_err:
                    min_err = err
                    temp = h
            if True:
                ...
                Wc, W = iter_model(df, "level", E, M, N)
                h = traverse_for_h(
                    [W], [Wc], z0, z1, q, w, E, M, N, T0=T0, n=-1, policy="level"
                )
                h_list = [temp]
