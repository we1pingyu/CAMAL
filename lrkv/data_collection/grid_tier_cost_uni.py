...
from utils.lsm import find_tier_T, find_tier_mbuf, tier_min_cost_sampling
...
class TierCost(object):
    ...
    def run(self):
        ...
        for workload in workloads:
            candidates = tier_min_cost_sampling(workload, 32)
            for candidate in candidates:
                ...
                # print(row)
                df.append(row)
                pd.DataFrame(df).to_csv('raw_data/tier_cost_grid_uniform_ckpt.csv')
                iter += 1

        self.logger.info('Exporting data from tier cost grid')
        pd.DataFrame(df).to_csv('raw_data/tier_cost_grid_uniform_min.csv')
        self.logger.info(
            f'Finished tier_cost_grid_uniform, use {time.time()-start_time}s\n'
        )
