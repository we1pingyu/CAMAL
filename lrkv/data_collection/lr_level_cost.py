        ...
        for workload in workloads:
            for size_ratio in size_ratios:
                for ratio in [0.25, 0.5, 0.75, 1.0]:
                    ...
                    key_path = 'key_log_level_cost_lr'
                    ...
                    # print(row)
                    df.append(row)
                    pd.DataFrame(df).to_csv('raw_data/level_cost_lr_ckpt.csv')
                    iter += 1

        self.logger.info('Exporting data from level cost')
        pd.DataFrame(df).to_csv('raw_data/level_cost_lr.csv')
        self.logger.info(f'Finished level_cost_lr, use {time.time()-start_time}s\n')
