    ...
    def run(self):
        ...
        for workload in workloads:
            ...
            while len(T_list) < num_samples:
                x = int(round(np.random.normal(temp, scale=num_samples)))
            ...
            for size_ratio in T_list:
                ...
                # print(row)
                df.append(row)
                pd.DataFrame(df).to_csv('raw_data/al_xgb_tier_cost_ckpt.csv')
                step += 1
            ...
            min_err = 1e9
            for h in range(1, 17):
                err = h_mbuf_tier_equation(h, z0, z1, q, w, T0)
                if err < min_err:
                    min_err = err
                    temp = h
            h_list = []
            while len(h_list) < num_samples / 2:
                x = int(round(np.random.normal(temp, scale=num_samples)))
        ...
        pd.DataFrame(df).to_csv('raw_data/al_xgb_tier_cost.csv')
