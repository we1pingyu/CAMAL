    ...
    def run(self):
        ...
        for i in range(0, 100):
            ...
            for num_samples in [20]:
                row = copy.deepcopy(row)
                row['optimizer'] = f'xgb_{num_samples}'
                level_cost_models = pkl.load(
                    open(f"model/level_cost_xgb_uni_{num_samples}.pkl", "rb")
                )
                (
                    best_T,
                    best_h,
            ...
            for num_samples in [20]:
                ...
                tier_cost_models = pkl.load(
                    open(f"model/tier_cost_xgb_uni_{num_samples}.pkl", "rb")
                )
        ...
        pd.DataFrame(df).to_csv('optimizer_data/xgb_optimizer_random_samples.csv')
