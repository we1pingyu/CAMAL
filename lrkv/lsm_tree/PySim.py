...
class RocksDB(object):
    ...
    def __init__(self, config):
        ...
        self.estimate_range_io_prog = re.compile(
            r'\[[0-9:.]+\]\[info\] \(estimate_range_io\) : ' r'\((-?\d+)\)'
        )
    ...
    def run(
        ...
        try:
            ...
            estimate_range_io_result = [
                float(result)
                for result in self.estimate_range_io_prog.search(proc_results).groups()
            ]
            ...
            results['estimate_range_io'] = estimate_range_io_result[0]
            return results
        except:
