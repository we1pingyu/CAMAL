...
all_samples = pd.read_csv('data/tier_cost_cache_ckpt.csv')
training_samples = all_samples[:300]
test_samples = all_samples[300:]
...
for _, sample in training_samples.iterrows():
    # if (sample['write_io'] + sample['blocks_read']) / Q < 10:
    # continue
    ...
    if sample['z1'] > 0.05 and sample['z1_cache_hit_rate'] < 1:
        Nf1 = sample['z1'] * Q * l * fpr * sample['T']
        Nf2 = sample['z1'] * Q * l * fpr
        Nf3 = sample['z1'] * Q
        xr = [1]
        for power in (alpha, c, Nf1, Nf2, Nf3, D, 1):
            for x in (alpha, c, Nf1, Nf2, Nf3, D):
                xr.append(power * np.log(x))
        Xr.append(xr)
        Yr.append(np.log(sample['z1_cache_hit_rate']))

    if sample['q'] > 0.05:
        Nf1 = sample['q'] * Q * l * sample['T']
        Nf2 = sample['q'] * Q * (S / B)
        xq = [1]
    ...
    z0 = sample['z0'] * l * fpr * sample['T'] * (1 - sample['z0_cache_hit_rate'])
    z11 = sample['z1'] * (1 - sample['z1_cache_hit_rate'])
    z12 = sample['z1'] * l * fpr * (1 - sample['z1_cache_hit_rate'])
    z13 = sample['z1'] * l * fpr * sample['T'] * (1 - sample['z1_cache_hit_rate'])
    q1 = sample['q'] * l * sample['T'] * (1 - sample['q_cache_hit_rate'])
    q2 = sample['q'] * (S / B) * (1 - sample['q_cache_hit_rate'])
    sample['N'] = sample['N'] + Q * sample['w']
    l = L(sample['N'], sample['mbuf'], sample['T'], False)
    w0 = sample['w'] * l * sample['T'] / B * (1 - sample['w_cache_hit_rate'])
    w1 = sample['w'] * l / B * (1 - sample['w_cache_hit_rate'])
    ...
    w5 = l / B * (1 - sample['w_cache_hit_rate'])
    w6 = l / sample['T'] * (1 - sample['w_cache_hit_rate'])
    w7 = sample['N'] * sample['T'] * (1 - sample['w_cache_hit_rate'])
    w8 = sample['N'] / sample['T'] * (1 - sample['w_cache_hit_rate'])
    w9 = np.log(sample['N'] * sample['T']) * (1 - sample['w_cache_hit_rate'])
    X.append([z0, z11, z12, z13, q1, q2, w4, w5, w6, w7, w8, w9, 1])
...
for _, sample in test_samples.iterrows():
    ...
    z0 = sample['z0'] * l * fpr * sample['T'] * (1 - ye)
    z11 = sample['z1'] * (1 - yr)
    z12 = sample['z1'] * l * fpr * (1 - yr)
    z13 = sample['z1'] * l * fpr * sample['T'] * (1 - yr)
    q1 = sample['q'] * l * sample['T'] * (1 - yq)
    q2 = sample['q'] * (S / B) * (1 - yq)
    sample['N'] = sample['N'] + Q * sample['w']
    l = L(sample['N'], sample['mbuf'], sample['T'], False)
    w0 = sample['w'] * l * sample['T'] / B * (1 - yw)
    w1 = sample['w'] * l / B * (1 - yw)
    w2 = sample['w'] * sample['mbuf'] * (1 - yw)
    w3 = sample['w'] * (1 - yw)
    w4 = sample['mbuf'] * (1 - yw)
    w5 = l / B * (1 - yw)
    w6 = l / sample['T'] * (1 - yw)
    w7 = sample['N'] * sample['T'] * (1 - yw)
    w8 = sample['N'] / sample['T'] * (1 - yw)
    w9 = np.log(sample['N'] * sample['T']) * (1 - yw)
    y_hat = np.dot([z0, z11, z12, z13, q1, q2, w4, w5, w6, w7, w8, w9, 1], W)
    y = (sample['write_io'] + sample['blocks_read']) / Q
    print(y_hat, y, sample['mbuf'], l * sample['T'], l / sample['T'])
    # print([z0, z11, z12, z13, q1, q2, w0, w1, w2, w3, w4, w5, w6, w7, w8, 1] * W)
