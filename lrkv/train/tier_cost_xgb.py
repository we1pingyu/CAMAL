...
# all_samples = pd.read_csv('raw_data/al_tier_cost_ckpt.csv')
# all_samples = all_samples.sample(frac=1)
# prepare_df(all_samples, 'prepared_data/al_tier_cost.csv')
all_samples = pd.read_csv('prepared_data/al_tier_cost.csv')
...
for i in range(10):
    ...
    # pickle.dump(cache_regr, open(f'model/al_tier_cache_xgb_{i}.pkl', "wb"))
    pickle.dump(regr, open(f'model/al_tier_cost_xgb_{i}.pkl', "wb"))
...
for _, sample in all_samples.iterrows():
    ...
    # pd.DataFrame(W).to_csv('data/tier_cost_weight.csv')
    # pd.DataFrame(Wc).to_csv('data/tier_cache_weight.csv')
